...
.status-ok {{
    background:#e8f5e9; padding:5px 10px; border-radius:10px;
}}
.status-warn {{
    background:#fff8e1; padding:5px 10px; border-radius:10px;
}}
.small-muted {{
...
with c1:
    st.image("assets/iteso_logo.png", width=300)
with c2:
    st.markdown("## Sistema de informes mensuales")
    st.caption("Dirección de Integración Comunitaria · ITESO")

st.divider()

# ---------- DEMO ACCESS ----------
with st.sidebar:
    st.markdown("### Acceso V1")
...
if profile == "Centro / Dirección":
    ...
    if page == "Nuevo reporte":
        ...
        st.caption(
            ...
            "Cada descripción admite un máximo de 250 palabras y sólo puede existir un Top 1, Top 2 y Top 3."
        )
        ...
        for i in range(st.session_state.num_activities):
            with st.expander(f"Actividad {i+1}", expanded=(i < 2)):
                ...
                with c1:
                    category = st.selectbox("Tema / categoría", CATEGORIES, key=f"cat_{i}")
                with c2:
                ...
                with c3:
                    participants = st.number_input(
                        ...
                        min_value=0, step=1, value=0, key=f"part_{i}"
                    )
                ...
                if title.strip() or desc.strip():
                    ...
                    if not title.strip():
                        errors.append(f"Actividad {i+1}: falta el nombre.")
                    if not desc.strip():
                        errors.append(f"Actividad {i+1}: falta la descripción.")
                    if wc > 250:
    ...
    if page == "Seguimiento mensual":
        ...
        if query.strip():
            ...
            for rep, a in hits:
                st.markdown(
                    f"""<div class="dic-card">
                    <b>{rep['unit_code']} · {rep['month']} {rep['year']}</b><br>
                    <span style="color:{ITESO_BLUE};font-size:1.05rem"><b>{a['title']}</b></span><br>
                    <span class="small-muted">{a.get('category','')} · {rank_label(a.get('ranking'))}</span><br><br>
                    {a.get('description_edited') or a.get('description_original') or ''}
                    </div>""",
                    unsafe_allow_html=True
                )
        else:
