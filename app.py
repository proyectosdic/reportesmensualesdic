...
def _ppt_activity_slide(prs, unit_code, activity, photos, page_num):
    ...
    _ppt_text(slide, body, 0.72, 2.18, 6.15, 3.7, size=14, color=(31, 41, 55))
...
def generate_consolidated_ppt(month, year, reports, activities_by_report):
    ...
    # Ranking slides first
    ranked = []
    for rep in reports:
        for act in activities_by_report.get(rep["id"], []):
            if selected_for_final(act["id"]) and act.get("ranking") in [1, 2, 3, "1", "2", "3"]:
                ranked.append((rep, act))
    ranked.sort(key=lambda item: int(item[1]["ranking"]))

    if ranked:
        section = prs.slides.add_slide(prs.slide_layouts[6])
        section.background.fill.solid()
        section.background.fill.fore_color.rgb = RGBColor(0, 59, 112)
        _ppt_text(section, "Principales hitos del mes", 0.75, 2.7, 11.5, 0.8, size=34, bold=True, color=(255, 255, 255))
        _ppt_text(section, f"{month} {year}", 0.78, 3.45, 8.0, 0.4, size=17, color=(225, 238, 247))
