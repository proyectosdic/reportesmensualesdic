...
def replace_activities(report_id, activities):
    """Replace activities and persist uploaded photos/charts."""
    if supabase:
        old_acts = (
        ...
        # Clean physical files for previous version before replacing it.
        old_paths = []
        for oa in old_acts:
            photo_rows = (
        ...
        for i, a in enumerate(activities, start=1):
            ...
            # Insert first so we have activity_id for folder structure.
            ...
            for photo in all_photos:
                try:
                    ...
                    photo_name = upload_name(photo, "fotografia.jpg")
            ...
            effective_chart = chart or a.get("existing_chart")
            if effective_chart:
                try:
                    ...
                    ext = Path(chart_original_filename).suffix.lower() or ".jpg"
                    unique_name = f"{uuid.uuid4().hex}{ext}"
                    chart_storage_path = f"{report_id}/{activity_id}/chart/{unique_name}"
                    supabase.storage.from_("dic-activity-photos").upload(
                        path=chart_storage_path,
                        file=chart_bytes,
                        file_options={
                            "content-type": chart_mime,
                            "upsert": "false",
                        },
                    )
                    ...
                    st.warning(f"No se pudo guardar la gráfica de '{a['title']}': {e}")
        return
...
def official_docx_template_path():
    return Path(__file__).parent / "assets" / DOCX_TEMPLATE_FILENAME
...
def parse_dic_docx(uploaded_bytes, expected_unit):
    ...
    center = general.get("CENTRO", "").strip().upper()
    month = general.get("MES", "").strip().title()
    year_raw = general.get("AÑO", "").strip()
    ...
    for i, table in enumerate(activity_tables[:5], start=1):
        ...
        title = values.get("NOMBRE_ACTIVIDAD", "").strip()
        category = values.get("CATEGORIA", "").strip()
        category_other = values.get("CATEGORIA_OTRO", "").strip()
        ranking_text = values.get("IMPORTANCIA", "").strip()
        participants_raw = values.get("PARTICIPANTES_ALCANCE", "").strip()
        description = values.get("DESCRIPCION", "").strip()
        social_url = values.get("RED_SOCIAL_URL", "").strip()
        chart_title = values.get("TITULO_GRAFICA", "").strip()

        chart_imgs = cell_images(row_by_label["GRAFICA"].cells[1], doc)
        photo_imgs = cell_images(row_by_label["FOTOGRAFIA"].cells[1], doc)

        # Completely unused activity: ignore.
        started = any([
        ...
        if category in CATEGORIES:
            ...
            other_category = ""
            content_warnings.append(f"Actividad {i}: CATEGORIA está vacía.")
        ...
        try:
            ...
            participants = 0
            content_warnings.append(
                f"Actividad {i}: PARTICIPANTES_ALCANCE debe ser un número entero."
            )
...
def hydrate_imported_docx(parsed):
    ...
    for i, act in enumerate(acts):
        ...
        st.session_state[f"rank_{i}"] = act.get("ranking_text") or "Sin ranking"
...
if profile == "Centro / Dirección":
    ...
    if page == "Nuevo reporte":
        ...
        if capture_method == "Carga desde archivo .docx" and not imported_ready:
            ...
            template_path = official_docx_template_path()
            if template_path.exists():
                ...
                st.error("No se encontró la plantilla oficial dentro de la aplicación.")
