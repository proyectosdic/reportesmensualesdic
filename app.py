...
def replace_activities(report_id, activities):
        ...
        for i, a in enumerate(activities, start=1):
            ...
            if chart:
                try:
                    ...
                    ext = Path(chart.name).suffix.lower() or ".jpg"
                    unique_name = f"{uuid.uuid4().hex}{ext}"
                    chart_storage_path = f"{report_id}/{activity_id}/chart/{unique_name}"
                    chart_original_filename = chart.name
                    supabase.storage.from_("dic-activity-photos").upload(
                        path=chart_storage_path,
                        file=chart_bytes,
                        file_options={
                            "content-type": getattr(chart, "type", "image/jpeg") or "image/jpeg",
                            "upsert": "false",
                        },
                    )
    ...
    for i, a in enumerate(activities, start=1):
        ...
        st.session_state.demo_activities.append({
            ...
            "chart_title": (a.get("chart_title") or "").strip() or None,
            "chart_storage_path": None,
            "chart_original_filename": getattr(chart, "name", None) if chart else None,
            "chart_bytes": chart.getvalue() if chart else None,
            "chart_mime_type": getattr(chart, "type", "image/jpeg") if chart else None,
            "order_index": i,
        })
        for photo in a.get("photos", []):
...
def validate_current_activities(num_activities):
    ...
    for i in range(num_activities):
        ...
        started = bool(
            ...
            or category_selected != CATEGORIES[0] or photos or social_url or chart or chart_title
        )
        if not started:
            continue
        ...
        if chart and not chart_title:
            missing.append("Título de la gráfica")
        if chart_title and not chart:
...
def current_uploaded_chart(activity):
    ...
    try:
        return {
            "bytes": chart.getvalue(),
            "mime_type": getattr(chart, "type", "image/jpeg") or "image/jpeg",
            "original_filename": getattr(chart, "name", "grafica.jpg"),
        ...
        for i in range(st.session_state.num_activities):
            ...
            with st.expander(f"Actividad {i+1}", expanded=(i < 2 and previous_complete)):
                ...
                if activity_started:
                    ...
                    if chart and not chart_title.strip(): missing.append("título de la gráfica")
