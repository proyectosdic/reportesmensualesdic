...
def invite_authorized_user(email, name="", unit_code="", role=""):
    """Send the Supabase Auth invitation email to a newly authorized user."""
    ...
    try:
        payload = {
            "email": email.lower(),
            "data": {"name": name, "unit_code": unit_code, "role": role},
        }
        response = requests.post(
        ...
        if response.status_code >= 400:
            ...
            return False, f"No se pudo enviar la invitación: {detail}", None
        data = response.json() if response.text else {}
        return True, "", (data or {}).get("id")
    except Exception as exc:
        return False, f"No se pudo enviar la invitación: {exc}", None


def send_password_recovery(email):
    email = (email or "").strip().lower()
    if not get_authorized_user(email):
        # Do not reveal whether an email is authorized.
        return True, "Si el correo está autorizado, recibirá instrucciones para restablecer su contraseña."
    url, headers = _auth_headers()
    try:
        response = requests.post(
        ...
        if response.status_code >= 400:
            return False, "No fue posible solicitar la recuperación de contraseña."
        return True, "Revisa tu correo institucional para restablecer la contraseña."
    except Exception:
        return False, "No fue posible solicitar la recuperación de contraseña."


def verify_auth_email_action(token_hash, action_type):
    if action_type not in {"invite", "recovery"}:
        return False, "El enlace no corresponde a una acción válida.", None
    url, headers = _auth_headers()
    try:
        response = requests.post(
            f"{url}/auth/v1/verify",
            headers=headers,
            json={"token_hash": token_hash, "type": action_type},
            timeout=12,
        )
        if response.status_code >= 400:
            return False, "El enlace venció o ya fue utilizado. Solicita uno nuevo.", None
        return True, "", response.json()
    except Exception as exc:
        return False, f"No fue posible validar el enlace. Detalle: {exc}", None


def set_password_with_access_token(access_token, new_password):
    url, headers = _auth_headers(access_token=access_token)
    try:
        ...
        if response.status_code >= 400:
            return False, "No fue posible establecer la contraseña. El enlace puede haber vencido."
        return True, ""
    except Exception as exc:
        return False, f"No fue posible establecer la contraseña. Detalle: {exc}"


def handle_password_setup_from_email():
    """Handle invite/recovery links made with TokenHash in Supabase email templates."""
    token_hash = st.query_params.get("token_hash", "")
    action_type = st.query_params.get("type", "")

    if token_hash and action_type in {"invite", "recovery"}:
        ok, msg, data = verify_auth_email_action(token_hash, action_type)
        if ok:
            st.session_state.password_setup_access_token = data.get("access_token", "")
            user = data.get("user") or {}
            st.session_state.password_setup_email = (user.get("email") or "").lower()
            st.session_state.password_setup_type = action_type
    ...
    if st.session_state.get("password_setup_access_token"):
        action = st.session_state.get("password_setup_type")
        st.header("Establecer contraseña" if action == "invite" else "Restablecer contraseña")
        email = st.session_state.get("password_setup_email", "")
        if email:
            st.info(f"Cuenta: **{email}**")
        p1 = st.text_input("Nueva contraseña", type="password", key="new_account_password")
        p2 = st.text_input("Confirmar contraseña", type="password", key="new_account_password_2")
        st.caption("Usa al menos 10 caracteres.")
        if st.button("Guardar contraseña", type="primary", use_container_width=True):
            if len(p1) < 10:
                ...
                ok, msg = set_password_with_access_token(
                    st.session_state.password_setup_access_token, p1
                )
                if ok:
                    st.success("Contraseña guardada. Ya puedes iniciar sesión.")
                    for key in ["password_setup_access_token", "password_setup_email", "password_setup_type"]:
...
def upsert_authorized_users(rows, send_invites=True):
    existing = {u["email"].lower(): u for u in list_authorized_users(include_inactive=True)}
    results = []
    now = datetime.utcnow().isoformat()
    for row in rows:
        previous = existing.get(row["email"])
        payload = {
            **row,
            "active": True,
            "updated_at": now,
        ...
        try:
            supabase.table("authorized_users").upsert(payload, on_conflict="email").execute()
            invite_status = "Actualizado"
            if send_invites and (not previous or not previous.get("auth_user_id")):
                ok, msg, auth_user_id = invite_authorized_user(
                    row["email"], row["name"], row["unit_code"], row["role"]
                )
                if ok:
                    update = {"invited_at": now, "updated_at": now}
                    if auth_user_id:
                        update["auth_user_id"] = auth_user_id
                    supabase.table("authorized_users").update(update).eq("email", row["email"]).execute()
                    invite_status = "Invitación enviada"
                else:
                    invite_status = msg
            results.append((row["email"], True, invite_status))
        except Exception as exc:
            results.append((row["email"], False, str(exc)))
    return results
...
# Invite and password-recovery links are handled before the normal login UI.
if handle_password_setup_from_email():
    st.stop()
...
if profile == "Centro / Dirección":
    with st.sidebar:
        if st.session_state.center_authenticated:
            ...
            with st.expander("¿Olvidaste tu contraseña?"):
                recovery_email = st.text_input("Correo para recuperación", key="recovery_email")
                if st.button("Enviar correo de recuperación", use_container_width=True):
                    ok, msg = send_password_recovery(recovery_email)
    ...
    if not email_validated:
        ...
        st.info(
            "Ingresa con el correo institucional que fue autorizado por Administración y tu contraseña."
        )
        st.stop()
    ...
    with st.sidebar:
        page = st.radio(
            "Menú",
            ["Seguimiento mensual", "Usuarios y accesos", "Informe consolidado", "Buscador histórico"]
        )
        st.divider()
    ...
    if page == "Seguimiento mensual":
        ...
        st.caption(
            "Administra quién puede ingresar, a qué centro pertenece y si tiene perfil de Colaborador o Director."
        )
        ...
        if users:
            ...
            for u in users:
                display_rows.append({
                    ...
                    "Invitación": format_access_datetime(u.get("invited_at")),
        ...
        send_invites = st.checkbox(
            ...
            value=True,
            help="Los usuarios nuevos recibirán un correo para establecer su contraseña.",
        )
        if uploaded_users is not None:
            ...
            if excel_errors:
                ...
                if st.button("Agregar / actualizar usuarios", type="primary", use_container_width=True):
                    results = upsert_authorized_users(parsed_users, send_invites=send_invites)
        ...
        if users:
            ...
            if selected_user:
                ...
                if not selected_user.get("last_login_at"):
                    if st.button("Reenviar invitación", use_container_width=True):
                        ok, msg, auth_user_id = invite_authorized_user(
                            selected_user.get("email"), selected_user.get("name", ""),
                            selected_user.get("unit_code", ""), selected_user.get("role", "")
                        )
                        if ok:
                            update = {"invited_at": datetime.utcnow().isoformat()}
                            if auth_user_id:
                                update["auth_user_id"] = auth_user_id
                            supabase.table("authorized_users").update(update).eq("email", selected_email).execute()
...
st.divider()
st.caption("Prototipo V1.26 · Dirección de Integración Comunitaria · ITESO")
