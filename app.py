...
def upsert_authorized_users(rows, generate_codes=True):
    """Add/update users. New or not-yet-activated users receive one-time activation codes."""
    ...
    for row in rows:
        ...
        try:
            ...
            if should_generate:
                ok, msg, code, expires = set_activation_code(row["email"])
                if ok:
                    status = "Código de activación generado"
                    generated_codes.append({
                        "Nombre": row.get("name", ""),
                        "Correo": row["email"],
                        "Centro": row["unit_code"],
                        "Rol": row["role"],
                        "Código": code,
                    ...
                    status = msg
            results.append((row["email"], True, status))
        except Exception as exc:
